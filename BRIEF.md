...
- **핵심 차별**: "내가 모은 진짜 콘텐츠 → 내 일정" = 멀티소스 레퍼런스 엔진. (Triple/MYRO=폼→AI생성, 위시빈=남의 일정 구경 과 구조적 차별)
...
- **톤 reference**: Triple 앱 (`../../triple_refs/`) — 둥근 카드, 슬롯 카드(사진+이름+"분류·권역"+추천 한 줄), 일자 탭, 하단 fixed CTA, 정적 지도 placeholder.
- **컬러**: 하늘(메인) + 코랄(액센트). Triple의 연두 자리를 하늘로.
...
> `index.html` = 전체 허브. 상단에 정체성/핵심루프/차별점 컨셉 섹션(팀원 이해도용) + 4그룹 카드 그리드(iframe 썸네일, 클릭 시 새 탭).
